...
def find_anomalous_transactions(
    ...
    Arguments:
        statement_file_path {str | Path} -- Path to the
    ...
    df_to_process = prepare_data_for_isolation_forest(
        statement_df, isolated_col
    )
    anomaly_df = find_anomaly(df_to_process, isolated_col, contamination)
    enriched_anomaly_df = enrich_anomaly_results(
        statement_df, anomaly_df, isolated_col
    )
