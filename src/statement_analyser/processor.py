...
from statement_analyser.constants import (
    HDFC_DEPOSITED_COL,
    HDFC_WITHDRAWAL_COL,
)
from statement_analyser.extractor import (
    extract_upi_description,
    extract_upi_name,
)
...
def process_upi_narration(df: pd.DataFrame) -> pd.DataFrame:
    """
    Process the 'Narration' column in the DataFrame to extract
...
def transform_transactions_df(transactions_df: pd.DataFrame) -> pd.DataFrame:
    transactions_df["Date"] = transactions_df["Date"].str.replace(
        "Sept", "Sep"
    )
...
def transform_paytm_transactions_df(
    transactions_df: pd.DataFrame,
) -> pd.DataFrame:
    transactions_df["Date_Formated"] = pd.to_datetime(
        transactions_df["Date"], format="%d/%m/%Y"
    ).dt.strftime("%d-%b-%Y")
    transactions_df["Time_Parsed"] = transactions_df["Time"].apply(
        parse_time_24
    )
...
def transform_statement_df(statement_df: pd.DataFrame) -> pd.DataFrame:
    ...
    statement_df["Date_Formated"] = statement_df["Date"].dt.strftime(
        "%d-%b-%Y"
    )
...
def filter_deposit_withdrawal(
    ...
    """
    Filter the statement and transactions DataFrames into
    ...
    """
    withdrawal_statements_df = statement_df[
        statement_df[HDFC_WITHDRAWAL_COL].notnull()
    ]
    withdrawal_transactions_df = transactions_df[
        transactions_df["Type"] == "DEBIT"
    ]
    deposit_statements_df = statement_df[
        statement_df[HDFC_DEPOSITED_COL].notnull()
    ]
    deposit_transactions_df = transactions_df[
...
def get_extended_statement(
    ...
    with the transactions DataFrame on matching dates and
